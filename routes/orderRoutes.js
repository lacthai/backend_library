...
router.post('/', async(req, res)=> {
  ...
  const {userId, cart, borrowDate, returnDate} = req.body;
  try {
    const user = await User.findById(userId);
    const order = await Order.create({owner: user._id, products: cart, borrowDate, returnDate});
    order.total = cart.length;
    await order.save();
    user.cart =  {total: 0};
